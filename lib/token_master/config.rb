  ...
  class Config
    ...
    private
      ...
      def get_option(key, option)
        ...
        # option = @options.dig([key, option]) ? @options.dig([key, option]) : DEFAULT_VALUES[option]
        # @options[key] ? @options[key].fetch(option, DEFAULT_VALUES[option]) : DEFAULT_VALUES[option]
      end
  end
end
